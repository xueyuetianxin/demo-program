  ...
  <div class="chat-detail">
    ...
    >
      ...
      >
        ...
        <div 
          v-if="message.msg_type == 6 || message.msg_type == '6'"
          class="card"
        >
          <div class="card-body">
            <img class="card-avater" :src="message.good?.store_good?.good_images[0]" alt="" srcset="">
            <div class="card-text">
              <div>{{ message.good?.name }}</div>
              <div>{{ message.good?.member_price }}</div>
            </div>
          </div>
        </div>

        <div 
          v-if="message.msg_type == '7'"
          class="order"
        >
          <div>订单号:{{ message.order_no }}</div>
          <div>下单时间:{{ message.order.created_at }}</div>
          <div>实付款:￥{{ message.order.price }}</div>
          <div>商品:{{ message.order.title }}</div>
        </div>

        <div 
          v-if="message.msg_type == 10 || message.msg_type == '10'"
          class="card"
          :class="{ 'card-center': true }"
        >
          <div class="card-body11" v-if="message.msg_content == null">
            <div>用户来自{{ store_name }}</div>
            <div class="good_card" v-if="message.good !== null">
              <img class="good-avater" :src="$formatAvatar(message?.good?.store_good?.good_images?.[0])" alt="" srcset="">
              <div class="good_detils">
                <div>{{ message?.good?.name }}</div>
                <div>￥{{ message?.good?.member_price }}</div>
              </div>
            </div>
          </div>
          <div class="card-body" v-if="message.msg_content !== null && message.service_user !== null">
            "{{ message?.service_user?.nickname }}"为您服务
          </div>
          <div class="card-body" v-if="message.service_user == null">
            {{ message.msg_content }}
          </div>
        </div>
...
import { apiServiceDetail , apiCustomerSendMessage ,apiUploadImage} from '@/api/user';
...
const handleSendMessage = async (messageText, mentionedUserIds) => {
  try {
    ...
    if (pendingImages.value.length > 0) {
      ...
      // 发送图片消息
      await apiCustomerSendMessage(message_id.value, {
        msg_type: 2, // 图片消息
...
const getSessionsDetail = async () => {
  ...
  if (!message_id) {
    console.error("message_id is undefined");
    return;
  }
  try {
    const res = await apiServiceDetail(message_id.value, {
      limit: 20,
      page: page.value,
      type: 1
    })
...
const loadMoreMessages = async () => {
  if (!message_id.value) {
    console.error("Cannot load more - message_id is undefined");
    loadingMore.value = false;
    return;
  }
  ...
  try {
    page.value += 1;
    await getSessionsDetail(); // 不再传递参数
  } catch (error) {
